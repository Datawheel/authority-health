      ...
      <SectionColumns>
        ...
        <article>
          ...
          {isZipLevelDataAvailable ? <p>In {topMostRaceData.Year}, {formatPercentage(topZipLevelData[topZipLevelData.RaceType])} of the population in {`zip ${topZipLevelData.Geography}`} were {lowerCaseRaceName(formatRaceNames(topZipLevelData.RaceType))} residents who visited health centers more than any other race/ethnicity group, compared to {formatPercentage(topMostRaceData[topMostRaceData.RaceType])} {lowerCaseRaceName(formatRaceNames(topMostRaceData.RaceType))} residents in Wayne County.</p> 
            : <p>{formatRaceNames(topMostRaceData.RaceType)} residents of Wayne County visit health centers more than any other race/ethnicity group ({formatPercentage(topMostRaceData[topMostRaceData.RaceType])} of all races). This is followed by {formatRaceNames(topSecondRaceData.RaceType)} residents ({formatPercentage(topSecondRaceData[topSecondRaceData.RaceType])}) and then {formatRaceNames(topThirdRaceData.RaceType)} residents ({formatPercentage(topThirdRaceData[topThirdRaceData.RaceType])}).</p>}
