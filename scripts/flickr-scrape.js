...
function fetchImage(row) {
  ...
  return flickr.photos.getInfo({photo_id: photoId})
    ...
    .then(([match, created]) => {
      ...
      if (row.display) payload.display = row.display;

      if (!created && shell.test("-e", splashPath) && shell.test("-e", thumbPath)) {
        return db.search.update(payload, {where: {id: row.id, dimension}})
          .then(printProgress);
      }
      else {
        ...
        return db.search
          .update(payload, {where: {id: row.id, dimension}})
...
const doc = new GoogleSpreadsheet("1PSn8dKRnAb3HYiHNd2-eQGNzy9OTYQodRLZ2cRxpBFo");
